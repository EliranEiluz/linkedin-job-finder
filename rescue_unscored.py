...
import search
from search import (
    _apply_claude_scoring, _apply_regex_fallback, _guest_session,
    _load_cv_text, BATCH_SIZE, claude_batch_score, fetch_description_guest,
    RESULTS_FILE, is_obviously_offtopic,
)
...
def main():
    ...
    for job in list(unscored):
        ...
        if reason and not job.get("priority"):
            ...
            unscored.remove(job)
    print(f"Title pre-filter removed {len([j for j in corpus if j.get('scored_by') == 'title-filter'])} obvious skips.")
    ...
    # Persist back (atomic write — replace the file rather than partial update).
